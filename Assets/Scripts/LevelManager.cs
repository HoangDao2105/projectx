...
public class LevelManager : MonoBehaviour
{
    ...
    IEnumerator CR_LoadLevelAsync(int level)
    {
        if (NextLevel == null && CurLevel == null)
        {
            AsyncOperationHandle<GameObject> loadWithIResourceLocations =
                Addressables.LoadAssetAsync<GameObject>("Assets/Prefabs/Levels/Level" + (level) + ".prefab");
    ...
    public void PreLoadNextLevel()
    {
        ...
        {
            AsyncOperationHandle<GameObject> loadWithIResourceLocations =
                Addressables.LoadAssetAsync<GameObject>("Assets/Prefabs/Levels/Level1.prefab");
            loadWithIResourceLocations.Completed += LoadWithIResourceLocations_Completed;
        }
        else
        {
            AsyncOperationHandle<GameObject> loadWithIResourceLocations =
                Addressables.LoadAssetAsync<GameObject>("Assets/Prefabs/Levels/Level" + (CurLevelIndex + 1) + ".prefab");
            loadWithIResourceLocations.Completed += LoadWithIResourceLocations_Completed;
        }
    }
    ...
    public AsyncOperationHandle<GameObject> LoadLastLevel()
    {
        ...
        AsyncOperationHandle<GameObject> loadWithIResourceLocations =
            Addressables.LoadAssetAsync<GameObject>("Assets/Prefabs/Levels/Level" + (CurLevelIndex)  + ".prefab");
        loadWithIResourceLocations.Completed += LoadWithIResourceLocations_Completed;
